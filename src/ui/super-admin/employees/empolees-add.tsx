...
import { Mail, Lock, Eye, EyeOff, Utensils, ChefHat, Calculator, Loader2 } from "lucide-react";
...
const ROLES = [
  {
    ...
    label: "Waiter",
    icon: Utensils,
    description: "Takes orders & serves",
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="grid grid-cols-3 gap-2">
        {ROLES.map(({ value, label, icon: Icon, description, color, bg, activeBorder }) => {
          ...
          return (
            ...
            >
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${isActive ? bg : "bg-white"}`}>
                <Icon className={`w-4.5 h-4.5 ${isActive ? color : "text-gray-400"}`} />
              </div>
              <div>
                <p className={`text-xs font-semibold ${isActive ? "text-gray-900" : "text-gray-600"}`}>
                  {label}
                </p>
  ...
  return (
    <FormProvider {...form}>
      <div className="px-1 py-2 flex flex-col gap-5 w-full">
        <FieldInput field="email" label="Email" type="email" placeholder="employee@example.com" icon={Mail} />
        <FieldInput field="password" label="Password" type="password" placeholder="Min. 6 characters" icon={Lock} />
        <RoleSelector />
