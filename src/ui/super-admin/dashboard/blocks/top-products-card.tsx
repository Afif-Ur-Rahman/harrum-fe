...
export const TopProductsCard = () => {
  return (
    <div className="overflow-hidden rounded-[28px] border border-white/10 bg-white/8 p-5 shadow-2xl shadow-black/25 backdrop-blur-xl sm:p-6">
      <div>
        <p className="text-sm tracking-[0.2em] text-emerald-200 uppercase">Best Performers</p>
        <h2 className="mt-2 text-2xl font-semibold text-white">Top Selling Products</h2>
      </div>

      <div className="mt-6 space-y-4">
        {topProducts.map((product, index) => (
          <div key={product.name} className="rounded-2xl border border-white/10 bg-black/20 p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="truncate text-base font-semibold text-white">
                  {index + 1}. {product.name}
                </p>
                <p className="mt-1 text-sm text-slate-400">{product.orders} orders</p>
              </div>
            ...
            <div className="mt-4 flex items-end justify-between gap-4">
              <div>
                <p className="text-xs tracking-[0.18em] text-slate-500 uppercase">Revenue</p>
                <p className="mt-1 text-xl font-semibold text-white">{product.revenue}</p>
              </div>
