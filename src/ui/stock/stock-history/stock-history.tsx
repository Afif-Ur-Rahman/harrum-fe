...
export const StockHistory = () => {
  ...
  const fetchHistory = useCallback(
    async (t = typeFilter, f = from, toDate = to) => {
      setLoading(true);
      setExpandedKey(null);
      setPage(1);
        ...
        <div>
          ...
          {searched && !loading && (
            <p className="text-sm text-gray-400 mt-1">
              {allSessions.length} batch{allSessions.length !== 1 ? "es" : ""} ·{" "}
      ...
      {!searched ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-sm text-gray-400">
            Choose dates above and press Search to view history.
          </p>
        </div>
      ) : loading ? (
        ...
        <div className="flex flex-col gap-4">
          <div className="space-y-2">
            {pagedSessions.map((session) => {
              ...
              const typeLabel =
                TYPE_TABS.find((t) => t.value === session.type)?.label ??
                session.type;
              ...
              return (
                ...
                >
                  ...
                  >
                    {/* Type badge */}
                    <span
                      className={`text-xs font-semibold px-2.5 py-1 rounded-full shrink-0 ${style.badge}`}
                    >
                      {typeLabel}
                    </span>
                    ...
                    {session.items[0]?.reason &&
                      session.items.every(
                        (i) => i.reason === session.items[0].reason,
                      ) && (
                        ...
                        <div className="divide-y divide-gray-100">
                          {session.items.map((item) => (
                            ...
                            >
                              ...
                              {item.reason &&
                                item.reason !== session.items[0]?.reason && (
                              ...
                              <span
                                className={`text-sm font-bold shrink-0 ${item.quantity < 0 ? "text-red-600" : "text-emerald-600"}`}
                              >
                                {item.quantity > 0 ? "+" : ""}
