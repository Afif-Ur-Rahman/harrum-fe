...
const StockIn = () => {
  ...
  const backHref =
    user?.type === "restaurant" ? "/restaurant/stocks" : "/accountant/stocks";

  const tableRef = useRef<HTMLDivElement>(null);

  const handleRemove = (name: string) => {
    const current = form.getValues("selectedStock") || [];
    form.setValue("selectedStock", current.filter((n) => n !== name));
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-xl font-bold text-gray-900">Stock In</h1>
              </div>
              <p className="text-xs text-gray-400 mt-0.5 ml-9">
                Add inventory quantities to existing stock items
              </p>
            </div>
          </div>
          ...
          <StockSearchAdd
            ...
            onSelect={(name) => {
              ...
              if (!current.includes(name))
                form.setValue("selectedStock", [...current, name]);
            }}
          />
          ...
          <div ref={tableRef} className="mt-4">
            {fields.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center rounded-2xl border-2 border-dashed border-gray-200 bg-white">
                ...
                <p className="text-sm font-semibold text-gray-700">No items selected</p>
                <p className="text-xs text-gray-400 mt-1">
                  Search above to add stock items
                </p>
              </div>
            ) : (
              <StockInTable
