        ...
        return (
          ...
          >
            ...
            <div className="flex items-center gap-2 border-b border-white/10 bg-white/8 px-4 py-3">
              ...
              <span className="ml-auto rounded-full bg-white/10 px-2.5 py-1 text-[10px] font-semibold tracking-widest text-slate-300 uppercase">
                {item.priceType}
              </span>
            </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-slate-400">
                      {item.quantity} pcs · {formatPrice(item.price ?? 0)} PKR
                    </p>
                  </div>

                  <div className="flex items-center gap-2">
                    {item.isReturned ? (
                      <span className="rounded-full bg-red-400/10 px-2.5 py-1 text-[11px] font-semibold text-red-300 ring-1 ring-red-300/30 ring-inset">
                        Returned
                      </span>
                    ) : (
                      <ConfirmationDialog
                        title="Return Item"
                        description={`Mark (${item.name}) as returned?`}
                        saveButtonTitle="Return"
                        confirmAction={() => onReturn(order._id, item._id!)}
                        trigger={
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {variant.isReturned ? (
                        ...
                        <span className="rounded-full bg-emerald-400/10 px-2.5 py-1 text-[11px] font-semibold text-emerald-300 ring-1 ring-emerald-300/30 ring-inset">
                          Claimed
                        </span>
                      ) : (
                        <>
                          <ConfirmationDialog
                            ...
                            confirmAction={() => onClaimItem(order._id, item._id!, variant._id!)}
                            trigger={
                          ...
                          <ConfirmationDialog
                            ...
                            saveButtonTitle="Return"
                            confirmAction={() => onReturnItem(order._id, item._id!, variant._id!)}
                            trigger={
                              <button className="rounded-lg border border-red-300/20 bg-red-400/10 px-3 py-1.5 text-xs font-semibold text-red-300 transition hover:bg-red-400/15">
                                Return
                              </button>
